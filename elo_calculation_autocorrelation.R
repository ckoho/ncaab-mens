...
year_elo_ratings <- function(df, df1, home_court, k){
  for (j in 1:nrow(df)) {
    ...
    if (df[[j, "win"]] == team2) {
      ...
      #Greater than.5 is team2 (home) is favorite, <.5 is team1 (road) favorite
      
      #MOV Multipler for auto correlation.
      mov <- 2.2 / ((400 * log ((1 - team2_win_per)/ team2_win_per))/
                      log(10)/2 * .001 + 2.2)
      adjust <- round(log(df[[j, "team2_pts"]] - df[[j, "team1_pts"]] +
                            1) * k  * mov * (1 - team2_win_per), 0)
      ...
      mov <- 2.2 / ((400 * log ((1 - (1 - team2_win_per))/ 
                                  (1 - team2_win_per)))/
                      log(10)/2 * .001 + 2.2)
      adjust <- round(log(df[[j, "team1_pts"]] - df[[j, "team2_pts"]] +
                            1) * k * mov * (team2_win_per), 0)
      df1[[which(df1 == team1, arr.ind=TRUE)[1], "elo"]] <- adjust +
        df1[[which(df1 == team1, arr.ind=TRUE)[1], "elo"]]
      df1[[which(df1 == team2, arr.ind=TRUE)[1], "elo"]] <- 
        df1[[which(df1 == team2, arr.ind=TRUE)[1], "elo"]] - adjust
